...
public class ArrayToolsTest {
	...
	private static void test04_a(int test_num, int length, int modulo) {
		...
		for(int test_cnt = 0; test_cnt < test_num; test_cnt++) {
			...
			//List<Item> items_b = IArrayTools.asList(items); // これだと Arrays.sort が走ってしまう。 // orig: //List<Item> items_b = IArrayTools.asList(items); // \u3053\u308c\u3060\u3068 Arrays.sort \u304c\u8d70\u3063\u3066\u3057\u307e\u3046\u3002
			///*
			...
			if(1 <= items.length &&
					items_a.get(0).equals(items_b.get(0)) == false && // コピーしているので、同じはず。 // orig: items_a.get(0).equals(items_b.get(0)) == false && // \u30b3\u30d4\u30fc\u3057\u3066\u3044\u308b\u306e\u3067\u3001\u540c\u3058\u306f\u305a\u3002
					items_a.get(0) == items_b.get(0) // items_a を複製しているので、異なるはず。 // orig: items_a.get(0) == items_b.get(0) // items_a \u3092\u8907\u88fd\u3057\u3066\u3044\u308b\u306e\u3067\u3001\u7570\u306a\u308b\u306f\u305a\u3002
					) {
				throw null; // test
			}
